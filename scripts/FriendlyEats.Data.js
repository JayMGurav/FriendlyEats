...
FriendlyEats.prototype.getFilteredRestaurants = function(filters, renderer) {
  ...
  */
  ...
  if(filters.city !== 'Any'){
    query = query.where('category', '==', filters.city);
  }
  if(filters.price !== 'Any'){
    query = query.where('category', '==', filters.price.length);
  }
  
  if(filters.sort === 'Rating'){
    query = query.orderBy('avgRating','desc');
  } else if(filters.sort === 'Reviews'){
    query = query.orderBy('numRating', 'desc');
  }
...
FriendlyEats.prototype.addRating = function(restaurantID, rating) {
  ...
  */
  ...
  var newRating  = document.collection('rating').doc();

  return firebase.firestore().runTransaction(function (transaction) {
    return transaction.get(document).then(function(doc){
      var data = doc.data();

      var newAvg = (data.numRatings * data.avgRating + rating.rating) /
      (data.numRatings + 1);
      ...
      return transaction.set(newRating, rating);
    });
  });
};
